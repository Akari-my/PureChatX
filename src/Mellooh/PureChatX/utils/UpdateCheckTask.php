...
final class UpdateCheckTask extends AsyncTask{
    ...
    public function onCompletion() : void{
        ...
        if(version_compare(UpdateChecker::normalize($remote), UpdateChecker::normalize($local), ">")){
            $server->getLogger()->notice("[PCX Checker] Your version of PureChatX is out of date.");
            $server->getLogger()->notice("[PCX Checker] Current: {$local}");
            $server->getLogger()->notice("[PCX Checker] Latest: {$remote}");
            $server->getLogger()->notice("[PCX Checker] Download Here!: https://github.com/Akari-my/PureChatX/releases");
        }else{
            // never mind
        }
    }
